<#
  gerar_apk.ps1  —  Gera o APK de release do Consulta de Preço (Windows).

  O que faz, em um comando:
    1. Confere se 'flutter' e 'keytool' estão disponíveis.
    2. Na primeira vez: cria a keystore de release e o android/key.properties.
       (a keystore e a senha ficam SÓ na sua máquina — nunca vão para o Git)
...
function Achar-Comando($nome) {
  $c = Get-Command $nome -ErrorAction SilentlyContinue
  if ($c) { return $c.Source }
  return $null
}

if (-not (Achar-Comando "flutter")) {
...
# --- 2) Keystore + key.properties (só na primeira vez) ------------------------
if (Test-Path $keyProps) {
  ...
  if (-not (Test-Path $keystore)) {
    ...
    & $keytool -genkeypair -v `
      ...
      -dname "CN=Consulta de Preco, OU=Pessoal, O=Pessoal, L=., S=., C=BR"
    if ($LASTEXITCODE -ne 0) { Write-Host "ERRO ao gerar a keystore." -ForegroundColor Red; exit 1 }
  } else {
...
flutter pub get
if ($LASTEXITCODE -ne 0) { Write-Host "ERRO no 'flutter pub get'." -ForegroundColor Red; exit 1 }
...
if ($LASTEXITCODE -ne 0) { Write-Host "ERRO no 'flutter build apk'." -ForegroundColor Red; exit 1 }
